        ...
        if (iPosEnd != std::string::npos)
        {
            ...
            if (iPosEnd != std::string::npos)
            {
                ...
                if (iPosEnd != std::string::npos)
                {
                    ...
                    cout<<"create Table succeed: sTableName:"<<sTableName<<" sColumns: "<<sColumns<<" sPrimaryKeys: "<<sPrimaryKeys;
//                    e.createTable(sTableName, createColVector(sColumns),
//                                  createVector(sPrimaryKeys));

                    vector  < tuple<string, string, bool> > testVector;
                    testVector=createColVector(sColumns);
                    for(int i=0;i<testVector.size();i++){
                        cout<<"i: "<<i<<" name: "<<get<0> (testVector[i])<<" type: "<<get<1> (testVector[i])<<endl;
                    }
                    return true;
                }
            }
